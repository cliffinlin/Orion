...
public class BarChartRenderer extends DataRenderer {
    ...
    protected void drawDataSet(Canvas c, BarDataSet dataSet, int index) {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            for (int j = 0; j < buffer.size(); j += 4) {
				// Modify for stock
				if (entries.get(j / 4).getVal() > 0) {
					mRenderPaint
							.setColor(dataSet.getIncreasingColor() == ColorTemplate.COLOR_NONE ? dataSet
									.getColor(j / 4) : dataSet
									.getIncreasingColor());

				} else {
					mRenderPaint
							.setColor(dataSet.getDecreasingColor() == ColorTemplate.COLOR_NONE ? dataSet
									.getColor(j / 4) : dataSet
									.getDecreasingColor());
