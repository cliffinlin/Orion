...
public class StockQuantAnalyzer {
	...
	void setupStockQuantBuy(Stock stock, StockData stockData, double price) {
		...
		stockQuant.setAction(StockDeal.ACTION_BUY);
...
//		stock.setQuantProfitMargin(stockQuant.getQuantProfitMargin());
	...
	void setupStockQuantSell(Stock stock, StockData stockData, double price) {
		...
		while (mBuyList.size() > 0) {
			...
			stockQuant.setAction(StockDeal.ACTION_SELL);
			...
			stockQuant.setupSellFee(mStockBonusList);
			stockQuant.setupNet();
			stockQuant.setupValue();
			stockQuant.setupProfit();
...
//			stock.setQuantProfitMargin(stockQuant.getQuantProfitMargin());
	...
	public void analyze(Context context, Stock stock, ArrayList<StockData> stockDataList, ArrayList<StockBonus> shareBonusList) {
		...
		if (stockDataList.size() < StockTrend.VERTEX_SIZE) {
			return;
		}

		if (shareBonusList == null || shareBonusList.size() < StockTrend.VERTEX_SIZE) {
//			return;
		}

		mStockBonusList = shareBonusList;
		mBuyList.clear();
